...
public class PvpOnLineConfig {
  ...
  /**对应场次*/
  public final int sceneMatch;

  /**精准匹配等级差*/
  public final int bestMatchLevelDiff;

  // @@@@@自定义属性开始区@@@@@

  // @@@@@自定义属性结束区@@@@@

  public PvpOnLineConfig(int id, String duan, String duanBigName, int duanBig, int duanSmall, int duanIconBig, int duanIconSmall, int starNum, int starCollect, float eloKValue, int scoreMax, int scoreProtection, int isScoreProtection, int winningStreak, int dropProtection, int duanProtection, int bestMatchScore, int bestMatchTime, int bestMatchStar, int fuzzyMatchScore, int fuzzyMatchTime, int fuzzyMatchStar, int leastMatchScore, int leastMatchTime, int leastMatchStar, int isTimeOutRebotMatch, int isLoseRobotMatch, int isPick, int winReward, int loseReward, int honorLimit, String awardWeek, String awardWeekPre, String bossTimeSetting, int robotPool, int sceneMatch, int bestMatchLevelDiff) {
    this.id = id;
    this.duan = duan;
    ...
    this.sceneMatch = sceneMatch;
    this.bestMatchLevelDiff = bestMatchLevelDiff;
  }
