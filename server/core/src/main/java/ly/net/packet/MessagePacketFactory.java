...
public class MessagePacketFactory {
    ...
    public static C2SMessagePacket createC2SMessagePacket(
            long guid, int cmd, AbstractMessage protoData, int seq, int sid) {
        C2SMessagePacket messagePacket =
                new C2SMessagePacket(guid, cmd, sid, seq, protoData.toByteArray());
        return messagePacket;
    }


    public static S2CMessagePacket createS2CMessagePacket(int cmd, int seq, byte[] data) {
        S2CMessagePacket messagePacket = createS2CMessagePacket(cmd, data);
        messagePacket.setSeq(seq);
        return messagePacket;
    }
