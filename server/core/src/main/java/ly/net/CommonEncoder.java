...
/**
 * Netty 出站编码器，把 {@link MessagePacket} 写成统一二进制包格式。
 * <p>
 * 真正的字段写入逻辑放在 packet 自身，编码器只负责接入 Netty pipeline 和统一日志。
 */
public class CommonEncoder extends MessageToByteEncoder<MessagePacket> {
  static final Logger log = LoggerDef.SystemLogger;

  @Override
  protected void encode(
      ChannelHandlerContext channelHandlerContext, MessagePacket packet, ByteBuf byteBuf)
      throws Exception {
